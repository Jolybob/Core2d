...
export class FarmWorldRenderer {
  ...
  destroy(): void {
    ...
    for (const key of appRuntime.worldRuntime.chunks.loadedKeys()) {
      const parts = key.split(',');
      if (parts.length !== 2) continue;
      const x = Number(parts[0]!); const y = Number(parts[1]!);
      if (Number.isFinite(x) && Number.isFinite(y)) appRuntime.worldRuntime.chunks.unload({ x, y });
    }
  ...
  private renderChunk(coord: ChunkCoord): Phaser.GameObjects.Graphics {
    ...
    for (let localY = 0; localY < CHUNK_SIZE; localY += 1) for (let localX = 0; localX < CHUNK_SIZE; localX += 1) {
      const worldX = coord.x * CHUNK_SIZE + localX; const worldY = coord.y * CHUNK_SIZE + localY;
  ...
  private syncResources(): void {
    ...
    for (const [key, view] of this.resourceObjects) {
      const resource = active.get(key); const chunk = resource && chunkKey(worldToChunk({ x: Math.floor(resource.x), y: Math.floor(resource.y) }));
      if (resource && chunk && appRuntime.worldRuntime.chunks.loadedKeys().has(chunk)) continue;
      view.object.destroy(); this.resourceObjects.delete(key);
    }
    for (const resource of active.values()) {
      const chunk = chunkKey(worldToChunk({ x: Math.floor(resource.x), y: Math.floor(resource.y) }));
      if (!appRuntime.worldRuntime.chunks.loadedKeys().has(chunk)) continue;
      ...
      if (!view) {
        ...
        view = { key: resource.key, x: resource.x, y: resource.y, type: resource.type, object }; this.resourceObjects.set(resource.key, view);
      }
