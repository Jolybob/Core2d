...
type ResourceView = { key: string; x: number; y: number; type: 'tree' | 'rock'; object: Phaser.GameObjects.GameObject & { visible: boolean } };
  ...
  private buildWorld(): void {
    ...
    for (const resource of appRuntime.resources.getAll()) {
      if (resource.type === 'tree') {
        const object = this.add.container(resource.x * TILE + 12, resource.y * TILE + 12).setDepth(5);
        object.add(this.add.rectangle(0, 10, 11, 22, 0x60452e));
        object.add(this.add.circle(0, -5, 17, 0x355d3b));
        this.resources.push({ key: resource.key, x: resource.x, y: resource.y, type: resource.type, object });
      } else {
        const object = this.add.rectangle(resource.x * TILE + 12, resource.y * TILE + 12, 17, 17, resource.ore ? 0xb7864f : 0x77736c).setDepth(3);
        this.resources.push({ key: resource.key, x: resource.x, y: resource.y, type: resource.type, object });
      }
    }
