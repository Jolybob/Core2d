...
type ResourceNode = { key: string; x: number; y: number; ore?: boolean; object: Phaser.GameObjects.GameObject };
...
export class CozyFarm extends Phaser.Scene {
  ...
  override create(): void {
    ...
    this.keys = {};
    for (const key of ['W', 'A', 'S', 'D', 'SHIFT', 'ONE', 'TWO', 'THREE', 'FOUR', 'FIVE', 'SIX', 'SEVEN', 'E', 'SPACE', 'F', 'B', 'L', 'Q', 'K', 'P']) {
      this.keys[key] = keyboard.addKey(key);
    }
    ...
    this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      if (pointer.leftButtonDown()) this.actionAt(pointer.worldX, pointer.worldY);
    });
  ...
  override update(_time: number, delta: number): void {
    ...
    if (dx || dy) {
      ...
      if (!this.blocked(nextX, nextY)) {
        appRuntime.dispatch({ type: 'MOVE', dx, dy, sprint, deltaSeconds: dt });
      }
    }
    ...
    this.updateNight(state.calendar.clock);
  }
  ...
  private buildWorld(seed: number): void {
    ...
    for (let i = 0; i < 30; i += 1) {
      ...
      const key = `tree:${x},${y}`;
      const object = this.add.container(x * TILE + 12, y * TILE + 12).setDepth(5);
      object.add(this.add.rectangle(0, 10, 11, 22, 0x60452e));
      object.add(this.add.circle(0, -5, 17, 0x355d3b));
      this.trees.push({ key, x, y, object });
    }
    for (let i = 0; i < 20; i += 1) {
      ...
      const ore = i % 3 === 0;
      const key = `rock:${x},${y}`;
      const object = this.add.rectangle(x * TILE + 12, y * TILE + 12, 17, 17, ore ? 0xb7864f : 0x77736c).setDepth(3);
      this.rocks.push({ key, x, y, ore, object });
    }
    for (let i = 0; i < 5; i += 1) {
      const animal = this.add.ellipse((22 + i * 2) * TILE + 12, 34 * TILE + 12, 19, 14, 0xf1dfbd).setDepth(12);
      this.tweens.add({ targets: animal, y: animal.y + 3, duration: 700 + i * 80, yoyo: true, repeat: -1 });
    }
  }

  private renderState(state: GameState): void {
    this.player?.setPosition(state.player.x, state.player.y);
    this.hud?.setText(`${SEASON_NAMES[state.calendar.season]} • DAY ${state.calendar.day}  $${state.player.money}\nHP ${Math.ceil(state.player.health)}  HUN ${Math.ceil(state.player.hunger)}  STA ${Math.ceil(state.player.stamina)}\n${state.player.tool.toUpperCase()} • ${state.calendar.weather}`);
    ...
    if (removedSignature !== this.lastRemovedSignature) {
      ...
      for (const node of [...this.trees, ...this.rocks]) {
        if (state.world.removedResources[node.key]) node.object.setVisible(false);
      }
    }
  }
  ...
  private actionAt(worldX: number, worldY: number): void {
    ...
    const playerX = Math.floor(state.player.x / TILE);
    const playerY = Math.floor(state.player.y / TILE);
    if (Math.hypot(x - playerX, y - playerY) > 4) return;
    const key = `${x},${y}`;

    switch (state.player.tool) {
      case 'hoe': appRuntime.dispatch({ type: 'TILL', key }); break;
    ...
    if (crop?.stage === 3 && state.player.tool === 'hoe') appRuntime.dispatch({ type: 'HARVEST', key });
  }

  private tryChop(x: number, y: number): void {
    const node = this.trees.find((entry) => Math.abs(entry.x - x) <= 1 && Math.abs(entry.y - y) <= 1);
    if (node) appRuntime.dispatch({ type: 'CHOP', resourceKey: node.key });
  }

  private tryMine(x: number, y: number): void {
    const node = this.rocks.find((entry) => Math.abs(entry.x - x) <= 1 && Math.abs(entry.y - y) <= 1);
    if (node) appRuntime.dispatch({ type: 'MINE', resourceKey: node.key, ore: Boolean(node.ore) });
  }

  private swing(): void {
    if (!appRuntime.dispatch({ type: 'ATTACK' })) return;
  ...
  private blocked(x: number, y: number): boolean {
    ...
    return tx >= 33 && tx <= 37 && ty >= 14 && ty <= 20;
  }
