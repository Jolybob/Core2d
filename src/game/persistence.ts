...
function migrate(raw: unknown): GameState | null {
  ...
  if (raw.schemaVersion === 3 && isGameState(raw.state)) return raw.state;
  return null;
}
...
export function loadGame(storage: Storage = localStorage): GameState | null {
  for (const key of [SAVE_KEY, ...LEGACY_KEYS]) {
    ...
    try {
      ...
      if (state) {
        if (key !== SAVE_KEY) storage.setItem(SAVE_KEY, JSON.stringify({ schemaVersion: 3, state } satisfies SaveData));
        return state;
      }
    } catch {
      // Ignore malformed saves and continue looking for another usable slot.
    }
  }
  return null;
}
