...
export class GameRuntime {
  ...
  dispatch(command: GameCommand): boolean {
    switch (command.type) {
      case 'TICK':
        ...
        return true;
      case 'MOVE':
        this.move(command.dx, command.dy, command.sprint);
        return true;
      case 'SELECT_TOOL':
        return this.selectTool(command.tool);
      case 'TILL':
        return this.farming.till(command.key);
      case 'PLANT':
        return this.farming.plant(command.key);
      case 'WATER':
        return this.farming.water(command.key);
      case 'HARVEST':
        return this.farming.harvest(command.key);
      case 'MINE':
        this.mine(command.resourceKey, command.ore);
        return true;
      case 'CHOP':
        this.chop(command.resourceKey);
        return true;
      case 'FISH':
        return this.fishing.catchFish();
      case 'ATTACK':
        return this.store.getState().inventory.sword > 0;
      case 'CRAFT':
        return this.crafting.craft(command.recipe);
      case 'EAT':
        return this.eat(command.item);
      case 'USE_SALVE':
        return this.useSalve();
      case 'BUY_SEEDS':
        return this.economy.buySeeds(command.cost, command.amount);
      case 'SHIP':
        return this.ship();
      case 'DAMAGE':
        return this.combat.damagePlayer(command.amount);
      case 'SAVE':
        return this.save();
      case 'LOAD':
        return this.load();
  ...
  private move(dx: number, dy: number, sprint: boolean): void {
    ...
    this.store.update((state) => {
      ...
      state.player.stamina = Math.max(0, state.player.stamina - dt * (sprint ? 12 : 3));
    });
  }
