...
export class FarmingSystem {
  ...
  private commitCrop(key: string): void { if (!this.runtime) return; const crop = this.getRuntimeCrop(cropEntityId(key)); if (!crop) return; this.store.update((state) => { state.world = this.runtime!.world; state.world.crops[key] = { stage: crop.stage, watered: crop.watered, tilled: crop.tilled }; }); }
  private commitAllCrops(): void { if (!this.runtime) return; const crops: Record<string, CropState> = {}; for (const entity of this.runtime.query.with('crop')) { const crop = this.getRuntimeCrop(entity.id); if (crop) crops[crop.key] = { stage: crop.stage, watered: crop.watered, tilled: crop.tilled }; } this.store.update((state) => { state.world = this.runtime!.world; state.world.crops = crops; }); }
}
