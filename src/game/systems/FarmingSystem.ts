...
/** Crop simulation is runtime-authoritative. GameState.world is an explicit persistence boundary. */
export class FarmingSystem {
  ...
  /** Explicit persistence boundary used when loading persisted crop state into the shared runtime. */
  ...
  harvest(key: string): boolean { const crop = this.getComponent(key); if (!crop || crop.stage < 3) return false; if (this.runtime) this.runtime.removeEntity(cropEntityId(key)); this.store.update((state) => { state.inventory.parsnip += 1; state.economy.totalHarvests += 1; delete state.world.crops[key]; }); this.events.publish({ type: 'CROP_HARVESTED', key }); return true; }
  ...
  private commitCrop(key: string): void { if (!this.runtime) return; const crop = this.getRuntimeCrop(cropEntityId(key)); if (!crop) return; this.store.update((state) => { state.world.crops[key] = { stage: crop.stage, watered: crop.watered, tilled: crop.tilled }; }); }
  private commitAllCrops(): void { if (!this.runtime) return; const crops: Record<string, CropState> = {}; for (const entity of this.runtime.query.with('crop')) { const crop = this.getRuntimeCrop(entity.id); if (crop) crops[crop.key] = { stage: crop.stage, watered: crop.watered, tilled: crop.tilled }; } this.store.update((state) => { state.world.crops = crops; }); }
}
