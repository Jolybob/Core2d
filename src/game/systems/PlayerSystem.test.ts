...
describe('PlayerSystem ECS', () => {
  it('creates the player as an ECS entity with position and player components', () => { const { runtime } = createPlayer(); const entity = runtime.query.with('player', 'position')[0]; expect(entity).toBeDefined(); expect(entity?.kind).toBe('player'); expect([...runtime.entities.ids()]).toHaveLength(1); });
