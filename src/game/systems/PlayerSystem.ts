...
export class PlayerSystem {
  ...
  constructor(private readonly store: GameStatePort, runtimeOrWorld: WorldRuntime | WorldSystem, compatibilityRuntime?: WorldRuntime) {
    this.runtime = compatibilityRuntime ?? (runtimeOrWorld instanceof WorldRuntime ? runtimeOrWorld : new WorldRuntime(this.store.getState().world));
  ...
  persist(): void {
    const entity = this.runtime.entities.get(this.playerId);
    const component = this.runtime.components.get<PlayerComponent>('player', this.playerId);
    if (!entity || !component) return;
    this.store.update((state) => {
      state.player.x = component.x; state.player.y = component.y; state.player.health = component.health;
      state.player.stamina = component.stamina; state.player.hunger = component.hunger; state.player.tool = component.tool;
      state.world.entities.entities[this.playerId] = { ...entity };
      const components = state.world.entities.components ??= {};
      components[this.playerId] = { player: { ...component }, position: { x: component.x, y: component.y } };
    });
  }
  ...
  move(dx: number, dy: number, sprint: boolean, deltaSeconds: number): boolean {
    ...
    this.setComponent(player); this.syncToState(player); return true;
  }
