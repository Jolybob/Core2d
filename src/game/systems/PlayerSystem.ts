...
export class PlayerSystem {
  ...
  eat(item: ConsumableId): boolean {
    ...
    const player = this.readComponent();
    player.hunger = Math.min(100, player.hunger + (item === 'parsnip' ? 35 : item === 'fish' ? 20 : 25));
    this.setComponent(player);
    this.syncToState(player);
    return true;
  }
  ...
  useSalve(): boolean {
    const player = this.readComponent();
    if (this.store.select((state) => state.inventory.salve < 1 || player.health >= 100)) return false;
