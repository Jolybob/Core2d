...
describe('ResourceSystem', () => {
  it('materializes deterministic resources as runtime ECS entities', () => {
    const { runtime, resources } = createResources();
    const nodes = resources.getAll();
    expect(nodes.length).toBeGreaterThan(0);
    expect(runtime.query.with('resource', 'position')).toHaveLength(nodes.length);
  ...
  it('removes the resource entity and records its removal in runtime persistence', () => {
    ...
    expect(runtime.query.with('resource', 'position').some((entity) => {
      const resource = runtime.components.get<{ key?: unknown }>('resource', entity.id);
      return resource?.key === tree.key;
    })).toBe(false);
    expect(runtime.isResourceRemoved(tree.key)).toBe(true);
    expect([...runtime.readPersistence((world) => Object.values(world.chunks))].some((chunk) => chunk.removedEntities[tree.key])).toBe(true);
  ...
  it('does not duplicate ECS resources when refreshed', () => {
    const { runtime, resources } = createResources();
    const before = runtime.query.with('resource', 'position').length;
    resources.refresh();
    expect(before).toBeGreaterThan(0);
    expect(runtime.query.with('resource', 'position')).toHaveLength(before);
  });
  ...
  it('uses the runtime loaded chunks instead of deriving a player-centered radius', () => {
    ...
    expect(runtime.isChunkLoaded({ x: 12, y: -9 })).toBe(true);
  });

  it('generates resources outside the former finite world rectangle', () => {
    const { runtime, resources } = createResources();
    const nodes = resources.getAll();
    const hasOutsideFiniteWorld = nodes.some((node) => node.x >= 70 || node.y >= 48 || node.x < 0 || node.y < 0);
    expect(nodes.length).toBeGreaterThan(0);
    expect(hasOutsideFiniteWorld).toBe(true);
    expect(runtime.loadedChunkCoords().length).toBeGreaterThan(0);
  });
});
