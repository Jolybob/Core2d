...
class RuntimeChunkManager {
  ...
  bindWorld(world: WorldState, preserveLoaded = false): void { const loaded = preserveLoaded ? [...this.loaded] : []; this.world = world; this.cache.clear(); this.loaded.clear(); for (const key of loaded) { const [x, y] = key.split(',').map(Number); if (Number.isFinite(x) && Number.isFinite(y)) this.load({ x, y }); } }
  ...
  loadedCoords(): ChunkCoord[] { return [...this.loaded].map((key) => { const [x, y] = key.split(',').map(Number); return { x, y }; }); }
...
export class WorldRuntime {
  ...
  ensureChunksAroundPixelPosition(position: PositionComponent, radius = 1): void { const center = worldToChunk({ x: Math.floor(position.x / TILE_SIZE), y: Math.floor(position.y / TILE_SIZE) }); const desired = new Set<ChunkKey>(); for (let y = center.y - radius; y <= center.y + radius; y++) for (let x = center.x - radius; x <= center.x + radius; x++) desired.add(chunkKey({ x, y })); for (const key of [...this.chunks.loadedKeys()]) if (!desired.has(key)) { const [x, y] = key.split(',').map(Number); if (Number.isFinite(x) && Number.isFinite(y)) this.unloadChunk({ x, y }); } for (const key of desired) { const [x, y] = key.split(',').map(Number); if (Number.isFinite(x) && Number.isFinite(y)) this.loadChunk({ x, y }); } }
