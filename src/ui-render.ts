...
const sameHud = (a: HudState, b: HudState) =>
  a.health === b.health &&
  a.hunger === b.hunger &&
  a.stamina === b.stamina &&
  a.money === b.money &&
  a.pickaxeLevel === b.pickaxeLevel &&
  a.tool === b.tool &&
  a.day === b.day &&
  a.weather === b.weather;

const sameInventory = (a: number[], b: number[]) =>
  a.length === b.length && a.every((value, index) => value === b[index]);

function renderHud(state: HudState, elements: UiRenderElements): void {
  elements.setText('[data-value="health"]', `${Math.ceil(state.health)} / 100`);
  elements.setBar('health', state.health);
  elements.setText('[data-value="hunger"]', `${Math.ceil(state.hunger)} / 100`);
  elements.setBar('hunger', state.hunger);
  elements.setText('[data-value="stamina"]', `${Math.ceil(state.stamina)} / 100`);
  elements.setBar('stamina', state.stamina);
  elements.setText('#ui-day', `DAY ${state.day}`);
  elements.setText('#ui-pickaxe', `Lv.${state.pickaxeLevel}`);
  elements.setText('#ui-tool', state.tool.toUpperCase());
  elements.setText('#ui-money', `$${state.money}`);
  elements.setText('#ui-weather', state.weather);
}
...
export function connectUiRenderers(elements: UiRenderElements): () => void {
  const unsubscribeHud = appRuntime.store.subscribe(
    (state) => ({
      health: state.player.health,
      hunger: state.player.hunger,
      stamina: state.player.stamina,
      money: state.player.money,
      pickaxeLevel: state.player.pickaxeLevel,
      tool: state.player.tool,
      day: state.calendar.day,
      weather: state.calendar.weather,
    }),
    (state) => renderHud(state, elements),
    sameHud,
  );

  const unsubscribeInventory = appRuntime.store.subscribe(
    (state) => ITEM_IDS.map((id) => state.inventory[id]),
    (counts) => renderInventory(counts, elements),
    sameInventory,
  );

  const unsubscribeCrafting = appRuntime.store.subscribe(
    (state) => ({
      counts: ITEM_IDS.map((id) => state.inventory[id]),
      pickaxeLevel: state.player.pickaxeLevel,
    }),
    (selection) => renderCrafting(selection.counts, selection.pickaxeLevel, elements),
